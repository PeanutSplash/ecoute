...
class BaseRecorder:
    ...
    def adjust_for_noise(self, device_name, msg):
        print(f"[信息] 正在调整来自 {device_name} 的环境噪声。" + msg)
        with self.source:
            self.recorder.adjust_for_ambient_noise(self.source)
        print(f"[信息] 已完成 {device_name} 的环境噪声调整。")
...
class DefaultMicRecorder(BaseRecorder):
    def __init__(self):
        ...
        self.adjust_for_noise("默认麦克风", "请通过默认麦克风制造一些噪声...")
...
class DefaultSpeakerRecorder(BaseRecorder):
    def __init__(self):
        ...
        self.adjust_for_noise("默认扬声器", "请通过默认扬声器制造或播放一些噪声...")